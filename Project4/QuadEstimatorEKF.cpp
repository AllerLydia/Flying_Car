...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
    ...
    rotation_matrix(0,1) = sin(phi) * tan(theta);
    rotation_matrix(0,2) = cos(phi) * tan(theta);
    rotation_matrix(1,1) = cos(phi);
    rotation_matrix(1,2) = -sin(phi);
    rotation_matrix(2,1) = sin(phi) * cos(theta);
    rotation_matrix(2,2) = cos(phi) * cos(theta);
    
    V3F update_rate = rotation_matrix *gyro;
    
    float predictedPitch = theta + dtIMU * update_rate.y;
    float predictedRoll = phi + dtIMU * update_rate.x;
    ekfState(6) = ekfState(6) + dtIMU * update_rate.z;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
    ...
    RbgPrime(0,2) = -cos(yaw) * sin(pitch) * sin(yaw) + sin(roll) * cos(yaw);
